...
export default async function ProtectedAdminLayout({
  ...
  const [kyc, payments, withdrawals, tickets, referralPayouts] = await Promise.all([
    ...
    prisma.referralPayout.count({
      where: { status: { in: ["PENDING", "UNDER_REVIEW", "APPROVED", "PROCESSING"] } },
    }),
  ]);

  return (
    <AdminShell user={user} counts={{ kyc, payments, withdrawals, tickets, referralPayouts }}>
      {children}
    </AdminShell>
  );
}
